...
@typechecked
def pbmc500(
    type: Literal["fastq", "bam", "fragment"] = "fragment", downsample: bool = False
) -> Path | list[Path]:
    ...
    if type == "fragment":
        if downsample:
            return Path(
                datasets.fetch("atac_pbmc_500_downsample.tsv.gz", progressbar=True)
            )
        else:
        ...
        return [
            Path(f)
            for f in datasets.fetch(
                "atac_pbmc_500_fastqs.tar", processor=pooch.Untar(), progressbar=True
            )
        ]
...
def colon() -> list[tuple[str, Path]]:
    ...
    """
    files = register_datasets().fetch(
        "colon_transverse.tar", progressbar=True, processor=pooch.Untar()
    )
...
def cis_bp(unique: bool = True) -> list[PyDNAMotif]:
    ...
    if unique:
        ...
        for motif in motifs:
            name = motif.name
            if (
                name not in unique_motifs
                or unique_motifs[name].info_content() < motif.info_content()
            ):
