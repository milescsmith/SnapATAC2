...
def recipe_10x_metrics(
    ...
    **kwargs
    ...
    qc["Mapping"]["Fraction_fragment_in_nucleosome_free_region"] = bam_qc[
        "frac_fragment_in_nucleosome_free_region"
    ]
    qc["Mapping"]["Fraction_fragment_flanking_single_nucleosome"] = bam_qc[
        "frac_fragment_flanking_single_nucleosome"
    ]
    ...
    qc["Targeting"]["Fraction_of_high-quality_fragments_overlapping_TSS"] = adata.uns[
        "frac_overlap_TSS"
    ]
    ...
    if peaks is None:
        ...
        peaks = [
            f"{row[0]}:{row[1]}-{row[2]}"
            for row in adata.uns["macs3_pseudobulk"].iter_rows()
        ]
    else:
    ...
    qc["Targeting"]["Fraction_of_genome_in_peaks"] = (
        snapatac2._snapatac2.total_size_of_peaks(peaks)
        / adata.uns["reference_sequences"]["reference_seq_length"].sum()
    )

    snapatac2.metrics.frip(adata, {"n_frag_overlap_peak": peaks}, normalized=False)
    qc["Targeting"]["Fraction_of_high-quality_fragments_overlapping_peaks"] = (
        adata.obs["n_frag_overlap_peak"].sum() / adata.obs["n_fragment"].sum()
    )

    cell_idx = snapatac2.pp.call_cells(
        adata, use_rep="n_frag_overlap_peak", inplace=False
    )
    ...
    if is_paired:
        qc["Cells"]["Mean_raw_read_pairs_per_cell"] = (
            bam_qc["sequenced_read_pairs"] / n_cells
        )
    else:
        qc["Cells"]["Mean_raw_read_pairs_per_cell"] = (
            bam_qc["sequenced_reads"] / n_cells
        )
    qc["Cells"]["Median_high-quality_fragments_per_cell"] = np.median(
        n_fragment[cell_idx]
    )
    qc["Cells"]["Fraction of high-quality fragments in cells"] = (
        n_fragment[cell_idx].sum() / n_fragment.sum()
    )

    adata.subset(cell_idx)
    frip = snapatac2.metrics.frip(
        adata,
        {"overlap_peak": peaks},
        normalized=False,
        count_as_insertion=True,
        inplace=False,
    )
    n_fragment = adata.obs["n_fragment"].to_numpy()
    if is_paired:
        qc["Cells"]["Fraction_of_transposition_events_in_peaks_in_cells"] = np.sum(
            frip["overlap_peak"]
        ) / (n_fragment.sum() * 2)
    else:
        qc["Cells"]["Fraction_of_transposition_events_in_peaks_in_cells"] = (
            np.sum(frip["overlap_peak"]) / n_fragment.sum()
        )
