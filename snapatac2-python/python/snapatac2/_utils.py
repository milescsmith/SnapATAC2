...
def is_anndata(data) -> bool:
    return (
        ...
        or isinstance(data, internal.AnnData)
        or isinstance(data, internal.AnnDataSet)
    )
...
def anndata_ipar(inputs, func, n_jobs=4):
    ...
    if exist_in_memory_adata:
        logging.warn(
            (
                "Input contains in-memory AnnData objects. "
...
def get_file_format(suffix):
    ...
    if suffix.endswith(".bw") or suffix.endswith(".bigwig"):
        ...
        _suffix.endswith(".bedgraph")
        or _suffix.endswith(".bg")
        or _suffix.endswith(".bdg")
    ):
        format = "bedgraph"
    else:
        format = None
...
def get_igraph_from_adjacency(adj):
    ...
    gr = ig.Graph(
        n=vcount, edges=edgelist, directed=False, edge_attrs={"weight": weights}
    )
    return gr
