...
@given(
    x=hnp.arrays(hnp.integer_dtypes(endianness="="), (500, 50)),
...
def test_make_fragment(datadir, tmp_path):
    ...
    snap.pp.make_fragment_file(
        bam, output, True, barcode_regex="(^[ATCG]+):", chunk_size=5000
    )
...
def test_reproducibility(mat):
    ...
    for _ in range(3):
        embeddings.append(
            snap.tl.spectral(adata, features=None, random_state=0, inplace=False)[1]
        )
    for x in embeddings:
    ...
    for _ in range(3):
        knn.append(
            snap.pp.knn(adata, random_state=0, n_neighbors=25, inplace=False).todense()
        )
    for x in knn:
    ...
    for _ in range(3):
        leiden.append(
            snap.tl.leiden(
                adata, random_state=0, resolution=1, n_iterations=10, inplace=False
            )
        )
    for x in leiden:
...
def read_bed(bed_file):
    with gzip.open(bed_file, "rt") as f:
        return sorted(
            [line.strip().split("\t")[:4] for line in f if line.startswith("chr")]
        )
...
def test_import(datadir):
    ...
    for fl in test_files:
        ...
        outputs = snap.ex.export_fragments(
            data, groupby="group", out_dir=str(datadir), suffix=".bed.gz"
        )
